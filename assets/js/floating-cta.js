...
(function(){
  ...
  var html = css +
    '<div id="snt-fab" class="snt-fab" role="region" aria-label="Quick contact">' +
      ...
      '</button>' +
    '</div>' +
    '<div id="snt-wechat-modal" class="snt-wcm" role="dialog" aria-label="Auf WeChat verbinden" onclick="if(event.target===this)this.classList.remove(\'is-open\');">' +
      '<div class="snt-wcm__box">' +
        '<button class="snt-wcm__close" onclick="document.getElementById(\'snt-wechat-modal\').classList.remove(\'is-open\');" aria-label="Schließen">×</button>' +
        '<h3 class="snt-wcm__title">Auf WeChat verbinden</h3>' +
        '<p class="snt-wcm__desc">QR-Code scannen oder WeChat-ID suchen:</p>' +
